...
React Hook Formとの一番の違いは、inputをライブラリ側に認識させるための登録の方法です。
React Hook Formはその名前にもある通り、Reactに大きく依存した設計となっており、register関数を使ってinput要素のrefをRHFが直接管理します。
対して、Conformではname属性を登録します。
...
重複コードにならないよう、`CustomFormControl`というコンポーネントを用意してまとめてしまっているため上記では見えませんが、この中では必須属性の有無でアイコンを出したり、エラーメッセージにアニメーションをつけるなど、フィールドに関する共通の処理を定義することができます。

`useField`はFormProviderを必要とするフックのため、それ自体もFormコンポーネントに組み込んで抽象化してしまいましょう。
...
```
フルのコードは[こちら]([https://](https://github.com/bmthd/react-experiment-repo/blob/master/src/ui/form/index.tsx))。
...
Submitせずに取得したFormDataをparseしてしまえば、サーバーフレームワークを使用しない場合でも同期的に値を取り出すことができます。
...
ソースコードも公開していますので、ぜひ設計の参考にして見てください。
